...
const SideBar = () => {
    return (
        <div className='h-[100vh] w-[100vw] sm:w-56 bg-sidebar-accent sm:rounded-r-3xl sm:border-r-2 border-accent flex flex-col justify-between'>
          <div>
            <h1 className='text-primary text-2xl text-center py-8  font-extrabold'>Budget Tracker</h1>
          ...
          <div className='flex justify-between p-6'>
            <button className='flex gap-2 text-primary hover:text-primary/80 ease-in-out border-2 p-1 rounded-lg hover:bg-primary/20 bg-secondary' onClick={()=> signOut()}><LogOut/>Logout</button>
            <ModeToggle/>
          </div>
        </div>
      )
